...
def agg_loss(loss_mat: torch.Tensor, loss_mask: torch.Tensor, loss_agg_mode: str):
    ...
    if loss_agg_mode == "token-mean":
    ...
    elif loss_agg_mode == "epic-token-mean":
        loss_mask_epic = torch.zeros_like(loss_mask)
        indices = loss_mask.nonzero(as_tuple=False)
        batch_ids = indices[:, 0]
        grouped_indices = [[] for _ in range(loss_mask.shape[0])]
        for i, idx in zip(batch_ids.tolist(), indices[:, 1].tolist()):
        ...
        for i in range(loss_mask.shape[0]):
            ...
            q1 = num_ones // 4
            q3 = num_ones - q1
            keep = idxs[:q1] + idxs[q3:]  # keep the first and last quarter
            loss_mask_epic[i, keep] = 1
            print(f"length is original is {num_ones}, after epic is {loss_mask_epic[i].sum().item()}")
        loss = verl_F.masked_mean(loss_mat, loss_mask_epic)
    else:
